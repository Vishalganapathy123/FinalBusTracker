        ...
        while (!Thread.currentThread().isInterrupted()) {
            try {
                ...
                for (Schedule schedule : schedules) {
                    ...
                    // Check 2: If trip hasn't ended and not already running
                    if (now.isBefore(arr) && !activeSchedules.contains(scheduleId)) {
                        
                        // FIX: Mark as active BEFORE starting thread (Prevents duplicate thread creation)
                ...
                // Check for new schedules every 10 seconds
                Thread.sleep(10000);
